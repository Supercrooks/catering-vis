var cca_left1 = echarts.init(document.getElementById("left1"),"dark");

var data = [{"name":"肺炎","value":1233},{"name":"实时","value":223},
    {"name":"新型","value":45}];
...
cca_left1.on("click",function (e) {
    // 联动地图
    new_mapPoints = [];
    for(var i = 0;i<mapPoints.length;i++){
        if(mapPoints[i].type === e.name){
            new_mapPoints.push(mapPoints[i]);
        }
    }
    map.clearOverlays(); //删除所有点
    overlays.length = 0;// 清空矩形区域
    // 遍历mapPoints创建标注点
    createMarks(new_mapPoints);
    // 联动折线图
    cca_left2_option_2.title.text = e.name;
    get_left2_data_2(e.name);
});